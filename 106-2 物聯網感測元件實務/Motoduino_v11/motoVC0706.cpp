...
#include "motoVC0706.h"

// Initialization code used by all constructor types
void motoVC0706::common_init(void) {
  swSerial  = NULL;
  hwSerial  = NULL;
  frameptr  = 0;
  bufferLen = 0;
...
#if ARDUINO >= 100
motoVC0706::motoVC0706(SoftwareSerial *ser) {
#else
motoVC0706::motoVC0706(NewSoftSerial *ser) {
#endif
...
boolean motoVC0706::setMotionStatus(uint8_t x, uint8_t d1, uint8_t d2) {
...
uint8_t motoVC0706::getMotionStatus(uint8_t x) {
  uint8_t args[] = {0x01, x};
...
boolean motoVC0706::setMotionDetect(boolean flag) {
...
boolean motoVC0706::getMotionDetect(void) {
  uint8_t args[] = {0x0};
...
uint8_t motoVC0706::getImageSize() {
...
boolean motoVC0706::setImageSize(uint8_t x) {
...
uint8_t motoVC0706::getDownsize(void) {
  uint8_t args[] = {0x0};
...
boolean motoVC0706::setDownsize(uint8_t newsize) {
...
char * motoVC0706::getVersion(void) {
  uint8_t args[] = {0x01};
...
void motoVC0706::OSD(uint8_t x, uint8_t y, char *str) {
...
boolean motoVC0706::setCompression(uint8_t c) {
  uint8_t args[] = {0x5, 0x1, 0x1, 0x12, 0x04, c};
  return runCommand(VC0706_WRITE_DATA, args, sizeof(args), 5);
}

uint8_t motoVC0706::getCompression(void) {
...
boolean motoVC0706::setPTZ(uint16_t wz, uint16_t hz, uint16_t pan, uint16_t tilt) {
...
boolean motoVC0706::getPTZ(uint16_t &w, uint16_t &h, uint16_t &wz, uint16_t &hz, uint16_t &pan, uint16_t &tilt) {
  uint8_t args[] = {0x0};
...
uint8_t motoVC0706::available(void) {
  return bufferLen;
}


uint8_t * motoVC0706::readPicture(uint8_t n) {
...
boolean motoVC0706::runCommand(uint8_t cmd, uint8_t *args, uint8_t argn, 
...
void motoVC0706::sendCommand(uint8_t cmd, uint8_t args[] = 0, uint8_t argn = 0) {
  if(swSerial) {
#if ARDUINO >= 100
...
uint8_t motoVC0706::readResponse(uint8_t numbytes, uint8_t timeout) {
  uint8_t counter = 0;
  bufferLen = 0;
  int avail;
...
boolean motoVC0706::verifyResponse(uint8_t command) {
