...
bool ESP8266::eATGMR(String &version)
{
    ...
    return recvFindAndFilter("OK", "AT", "\r\n\r\nOK", version); 
}

String ESP8266::eATGMR()
{
	  String version;
    rx_empty();
//    m_puart->println("AT+GMR");
    version = sendData("AT+GMR\r\n",2000, ESP_DEBUG);
    
    return getSubString(version, "version:");
...
bool ESP8266::sATCWJAP(String ssid, String pwd)
{
 ...
 cmd+="\"\r\n";
  ...
  list = sendData(cmd, 1000, 1);
  
  bool eOK = findString(list, "ERROR");
  if(findString(list, "ERROR"))
  	return false;
  else	
    return true;
}
...
String ESP8266::sATCIPSTA_CUR(uint32_t timeout)
{
    rx_empty();

//    String resp = sendData("AT+CIPSTA_CUR?\r\n",timeout,1); 
    
    return sendData("AT+CIPSTA_CUR?\r\n",timeout, ESP_DEBUG);
}
...
String ESP8266::sendData(String command, const int timeout, boolean debug)
{
    String response = "";    
    m_puart->print(command); // send the read character to the esp8266
    ...
    while( (time+timeout) > millis())
    {
      while(m_puart->available())
      {
        ...
        char c = m_puart->read(); // read the next character.
        response+=c;
      }  
    }
...
bool ESP8266::findString(String list, String target)
{
    String data_tmp;
...
String ESP8266::extractIP(String list)
{
       String ipaddress;
       
       int ipIndex_1st = list.indexOf('\"');
       int ipIndex_2nd = list.indexOf('\"',ipIndex_1st+1);
       ipaddress = list.substring(ipIndex_1st+1, ipIndex_2nd); 
       
       return ipaddress;
}

String ESP8266::receiveData(uint32_t timeout)
{
	  uint8_t esp_buffer[128] = {0};
    String inData="";
    rx_empty();
    
    if(recvPkg(esp_buffer, 128, NULL, timeout, 0) == 0)
    {
      return "";
      }
    else {
          String inData((const char*)esp_buffer);
...
String ESP8266::getSubString(String list, String nextString)
{
      // String subString;
       
       int ipIndex_1st = list.indexOf(nextString);
       int ipIndex_2nd = list.indexOf('\n',ipIndex_1st+nextString.length());
      // subString = list.substring(ipIndex_1st+1, ipIndex_2nd); 
       
       return list.substring(ipIndex_1st+nextString.length(), ipIndex_2nd); ;
}